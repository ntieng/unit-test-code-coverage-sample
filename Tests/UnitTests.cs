...
namespace Coffee.Api.Tests
{
	public class Tests
	{
        ...
        public async Task TestSuccess()
        {
            ...
            var activity = await client.GetFromJsonAsync<CoffeeResponseModel>("/Coffee");
                ...
                .WithWebHostBuilder(builder =>
                {
                    builder.ConfigureServices(services =>
                    {
                        ServiceDescriptor serviceDescriptor = new(typeof(IBackendService),
                                typeof(CoffeeService), ServiceLifetime.Scoped);
                        services.Remove(serviceDescriptor);
                        services.AddScoped<IBackendService>(s => new CoffeeService(mockHttpClient));
                    });
                });

            var client = application.CreateClient();

            var responseMessage = await client.GetAsync("/Coffee");
